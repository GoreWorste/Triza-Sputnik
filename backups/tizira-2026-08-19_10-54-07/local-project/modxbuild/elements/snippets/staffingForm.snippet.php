<?php
/**
 * staffingForm — заявка на подбор персонала (почта + файл + капча).
 */
...
if (empty($_SESSION['tz_captcha_a']) || empty($_SESSION['tz_captcha_b'])) {
    $_SESSION['tz_captcha_a'] = random_int(2, 9);
    $_SESSION['tz_captcha_b'] = random_int(1, 8);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['tz_form'] ?? '') === 'staffing') {
    ...
    $cap = (int)($_POST['captcha'] ?? -1);
    $expect = (int)$_SESSION['tz_captcha_a'] + (int)$_SESSION['tz_captcha_b'];
    if ($cap !== $expect) $errors[] = 'Неверный ответ на проверочный вопрос.';
...
$a = (int)$_SESSION['tz_captcha_a'];
$b = (int)$_SESSION['tz_captcha_b'];
...
$out .= '<form class="tz-staff-form__form" method="post" enctype="multipart/form-data" action="#tz-staff-form" novalidate>';
...
$out .= '<label class="tz-staff-form__field tz-staff-form__field--full"><span>Описание задачи *</span><textarea name="message" rows="8" required>' . $e($old['message']) . '</textarea></label>';
...
$out .= '<label class="tz-staff-form__field tz-staff-form__field--captcha"><span>Проверка: сколько будет ' . $a . ' + ' . $b . '? *</span><input type="number" name="captcha" required inputmode="numeric" autocomplete="off" placeholder="Введите ответ"></label>';
$out .= '</div>';
$out .= '<button type="submit" class="tz-btn tz-btn--glow">Отправить заявку</button>';
