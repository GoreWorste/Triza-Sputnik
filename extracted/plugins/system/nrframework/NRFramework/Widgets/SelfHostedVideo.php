...
class SelfHostedVideo extends Video
{
	...
	{
		// By default, set the default video to the given value
		$value = isset($this->options['value']) && !empty($this->options['value']) ? $this->options['value'] : false;
		$this->options['video']['file'] = $value;
		
		// If a value is set, then try to find a self hosted video if possible
		// This allows us to find the exact video file (mp4/webm/ogg/mov) extension
		if ($value)
		{
			$selfHostedVideoDetails = \NRFramework\Helpers\File::getFileSources($this->options['value'], ['mp4', 'webm', 'ogg', 'mov']);
			if ($selfHostedVideoDetails)
			{
				$this->options['video'] = is_array($selfHostedVideoDetails) && isset($selfHostedVideoDetails[0]) ? $selfHostedVideoDetails[0] : false;
			}
		}
